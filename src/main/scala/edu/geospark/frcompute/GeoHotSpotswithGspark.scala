...
object GetHotSpots 
{
  ...
  def main(args: Array[String]): Unit = 
  {
    ...
    println("Filtered points " +filteredPointRDD.rawSpatialRDD.count())
    
    // Get the rectangle RDD
    val boundaryEnvelope = newyorkEnvelope
    ...
    for (i <- 1 to 31)
    {
      ...
      val tempJoinRDD = JoinQuery.SpatialJoinQueryCountByKey(pointDayRdd, rectangleRDD, false, true).rdd
                                    .map(x => ((x._1.getCoordinates()(0).x, x._1.getCoordinates()(0).y, i), x._2.toLong))
      println("Temp RDD : " + i + " "  + tempJoinRDD.count())
      println(tempJoinRDD.map(x => x._2).reduce((x, y) => x+ y))
      joinResults = joinResults.union(tempJoinRDD)
      println("Join Result RDD : " + joinResults.count())
    }
    dayRDD.unpersist();
    ...
    val intermean = (joinResults.map(x => x._2).reduce((x, y) =>  x + y)).toDouble 
    println("Total joinResults reduced value " + intermean)
