...
object GetHotSpots 
{
  ...
  def main(args: Array[String]): Unit = 
  {
    ...
    def getNeighbourValues(x : Double, y : Double, day : Int) : Long =
    {
      var count : scala.Long = 0
      val countsMap = pointsCountMapBC.value
      def checkXY(x : Double, y : Double, z: Int) : Boolean = 
        if(x >= minXBC.value && x <= maxXBC.value && y >= minYBC.value && y <= maxYBC.value && z >= 1 && z <= 31 && countsMap.contains((x, y, z))) true else false
      for( i <- 1 to xArrayBC.value.length -1)
      {
    ...
    val horizontalPartitions = Math.ceil((boundaryEnvelope.getMaxX - boundaryEnvelope.getMinX) / 0.01).toInt
		...
		val numCells = horizontalPartitions * verticalPartitions.toDouble
    ...
    val weightBC = sc.broadcast(weight)
    val weightSquareBC = sc.broadcast(weightSquare)
    ...
    for (i <- 1 to 31)
    {
      finalResults = finalResults.union(squaresRDD.map 
                                  { x => 
                                    val neighbourValues = getNeighbourValues(x._1, x._2, i)
                                    val numerator : Double = neighbourValues - (meanBC.value * weightBC.value)
                                    val denominatorRight : Double = ((numCellsBC.value * weightSquareBC.value) - (weightBC.value * weightBC.value)) / (numCellsBC.value - 1)
    ...
    finalResults.sortBy(_._2, false).take(50).foreach(x => println("Lat : " + x._1._1 + ", Long : " + x._1._2 + ", Day : "+ x._1._3 + ", Score : " + x._2))
  }
